...
| `GOOGLE_OAUTH_CLIENT_ID` | `src/app/api/content/todos/sync/route.ts`<br />`src/app/api/settings/connections/route.ts`<br />`src/app/api/storage/download/route.ts`<br />`src/lib/auth.ts`<br />`src/lib/auth/social-auth.ts`<br />`src/lib/integrations/GoogleDriveAdapter.ts` |
...
| `GEN_LANG_API_KEY` | `src/environment.ts`<br />`src/lib/features/ai/providers/GeminiProvider.ts`<br />`src/lib/rag/vector-database.ts` |
