...
export class RAGSystem {
  ...
  async addDocument(
    ...
    try {
      ...
      const docRef = doc(collection(getFirebaseFirestore(), this.config.documentsCollection), documentId);
      ...
      for (const chunk of chunks) {
        await addDoc(collection(getFirebaseFirestore(), this.config.chunksCollection), {
          ...chunk,
  ...
  async getDocument(documentId: string, userId?: string): Promise<RAGDocument | null> {
    try {
      ...
      const docRef = doc(getFirebaseFirestore(), this.config.documentsCollection, documentId);
    ...
    try {
      ...
      if (updates.content) {
        ...
        const chunksQuery = firestoreQuery(
          collection(getFirebaseFirestore(), this.config.chunksCollection),
  ...
  async deleteDocument(documentId: string, userId: string): Promise<boolean> {
    try {
      ...
      await deleteDoc(doc(getFirebaseFirestore(), this.config.documentsCollection, documentId));
  ...
  async queryDocuments(
    ...
    try {
      ...
      // Build query
      let docsQuery: any = collection(getFirebaseFirestore(), this.config.documentsCollection);
